...
const state = {
  view:"discover", eventId:null,
  chip:null, q:"", radius:null, city:"Toronto",
  orders: load("ev_orders", []),
  saved:  load("ev_saved", []),
  expanded:false, locOpen:false,
};
...
/* ---------- filtering ---------- */
function inRadius(e){ return state.radius === null || e.km <= state.radius }
const has = (e,t) => e.tags.includes(t);

function parseQuery(q){
  const s = q.toLowerCase();
  const f = {tags:[], maxPrice:null, text:s};
  ...
  if(/food|eat|restaurant|festival/.test(s)) f.tags.push("food");
  if(/music|concert|dj|dance|party/.test(s)) f.tags.push("music");
  if(/comedy|funny|standup|stand-up/.test(s)) f.tags.push("comedy");
  if(/cheap|budget|under \$?25|affordable/.test(s)) f.maxPrice = 25;
  const m = s.match(/under \$?(\d+)/); if(m) f.maxPrice = +m[1];
  return f;
}
/* Words that only ever describe the *shape* of a query, never an event name. */
const STOP = new Set(["under","over","near","this","that","near","events","event","things",
  "activities","activity","stuff","something","the","for","and","with","what","where","when",
  "around","here","cheap","budget","free","today","tonight","weekend","kids","kid","family"]);

function searchResults(){
  if(!state.q.trim()) return [];
  const f = parseQuery(state.q);
  const structured = f.tags.length > 0 || f.maxPrice !== null;
  const words = f.text.replace(/[^a-z0-9\s]/g," ").split(/\s+/)
    .filter(w => w.length > 2 && !STOP.has(w));
  return EVENTS.filter(e=>{
    if(!inRadius(e)) return false;
    if(f.maxPrice !== null && e.from > f.maxPrice) return false;
    if(f.tags.length && !f.tags.some(t=>has(e,t))) return false;
    if(structured) return true;
    const hay = (e.title+" "+e.cat+" "+e.venue+" "+e.area+" "+e.blurb).toLowerCase();
    return words.length ? words.some(w=>hay.includes(w)) : false;
  });
}

const SECTIONS = [
  {k:"🔥", t:"Trending near you", n:"Most saved this week", f:e=>e.trending},
  {k:"", t:"This weekend",       n:"Friday to Sunday",     f:e=>has(e,"weekend")},
  {k:"", t:"Under $25",          n:"Big nights, small spend", f:e=>e.from>0 && e.from<25},
  {k:"", t:"Free to attend",     n:"No ticket needed",     f:e=>has(e,"free")},
  {k:"", t:"Food & festivals",   n:"Eat your way around the city", f:e=>has(e,"food")},
  {k:"", t:"Music & nightlife",  n:"Late sets and live bands", f:e=>has(e,"music")},
  {k:"", t:"Family & kids",      n:"Everyone's welcome",   f:e=>has(e,"family")},
  {k:"", t:"Something different",n:"Things you wouldn't think to search", f:e=>has(e,"different")},
];
const CHIPS = [
...
function go(view, id){
  state.view = view;
  if(id){ state.eventId = id; state.expanded = false }
  render();
...
function card(e, wide){
  ...
  const socialTag = e.trending ? `<span class="tagpill flame">🔥 Trending</span>`
  ...
  <article class="ecard${wide?" wide":""}">
    <button class="ehit" onclick="go('event','${e.id}')" aria-label="${esc(e.title)}">
      <div class="eart" style="${art(e)}">
        ...
        <div class="vibe">${socialTag}${e.video?`<span class="tagpill solid">▶ Clip</span>`:""}</div>
      </div>
      <div class="ebody">
        <div class="when">${e.dateShort} · ${e.time}</div>
        <h3>${esc(e.title)}</h3>
        <div class="eplace">${I.pin} ${esc(e.venue)}</div>
        <div class="efoot">
...
function reel(e){
  ...
  <button class="reel" onclick="go('event','${e.id}')" aria-label="${esc(e.title)} — watch clip">
    ...
    <div class="scrim"></div>
    <span class="reelplay">${I.play}</span>
    <span class="reellen">${e.vlen}</span>
    <div class="reelinfo">
      <div class="rt">${esc(e.title)}</div>
      <div class="rd">${e.dateShort} · ${esc(e.area)}</div>
...
function header(compact){
  const radiusLabel = state.radius ? `Within ${state.radius} km` : "All of Toronto";
  return `
  <div class="tbwrap">
    ...
    <div class="locrow">
      <button class="locpick" onclick="state.locOpen=!state.locOpen; render()" aria-expanded="${state.locOpen}">
        <span class="pinico">${I.pin}</span>${state.radius?"Near Me":"Toronto"}<span class="caret">▾</span>
      </button>
      <span style="font-size:12.5px;color:var(--ink-3);font-weight:600">${radiusLabel}</span>
    </div>
    <nav class="desknav">
  ...
  ${state.locOpen ? locSheet() : ""}`;
}
function locSheet(){
  const opts = [null,5,10,25,50];
  return `<div class="chiprail" style="padding-top:0">
    ${opts.map(r=>`<button class="chip${state.radius===r?" on":""}" onclick="state.radius=${r}; state.locOpen=false; render()">
      ${r===null?"All of Toronto":r+" km"}</button>`).join("")}
  </div>`;
}

/* ---------- views ---------- */
function render(){
  document.body.classList.toggle("on-event", state.view==="event");
  el("topbar").innerHTML = state.view==="discover" ? header(false)
                         : state.view==="search"   ? header(true) : "";
  el("topbar").style.display = (state.view==="discover"||state.view==="search") ? "" : "none";
  const v = {discover:discoverView, search:searchView, saved:savedView,
             tickets:ticketsView, profile:profileView, event:eventView, confirm:confirmView}[state.view];
  el("app").innerHTML = v();
  el("tabbar").innerHTML = tabbar();
  if(state.view==="confirm") drawQRs();
  if(state.view==="search"){ const i = el("qin"); if(i && !state._noFocus) i.focus(); state._noFocus=false }
}

function discoverView(){
  const pool = EVENTS.filter(inRadius);
  if(state.chip){
    const list = pool.filter(e=>has(e,state.chip) || (state.chip==="trending"&&e.trending));
    return `${chipRail()}
      <div class="phead" style="padding-top:14px"><h1>${CHIPS.find(c=>c.id===state.chip).label.replace("🔥 ","")}</h1>
      <p>${list.length} event${list.length===1?"":"s"} ${state.radius?`within ${state.radius} km`:"in Toronto"}</p></div>
      ${list.length?`<div class="grid2">${list.map(e=>card(e,true)).join("")}</div>`:emptyBox("Nothing here right now","Try a wider radius or a different vibe.")}
      ${demoBar()}`;
  }
  ...
  return `
    ...
    <section class="sec">
      ...
      <div class="rail">${reels.map(reel).join("")}</div>
    </section>
    ${SECTIONS.map(s=>{
      ...
      return `<section class="sec">
        ...
        <div class="rail">${list.map(e=>card(e)).join("")}</div>
      </section>`;
    }).join("")}
    ${demoBar()}`;
}
...
const SUGGESTIONS = [
  {q:"Things to do this weekend", e:"🗓️"},
  {q:"Free events near me", e:"🎈"},
  {q:"Food festivals in Toronto", e:"🍜"},
  {q:"Kids activities Sunday", e:"🧒"},
  {q:"Live music tonight", e:"🎧"},
];
...
function searchView(){
  ...
  <div class="pad" style="padding-top:4px">
    <div class="searchfield">
      ...
      <input id="qin" value="${esc(state.q)}" placeholder="Try “free events this weekend”"
        oninput="state.q=this.value; state._noFocus=true; render()" aria-label="Search">
  ...
  ${!state.q.trim() ? `
    <div class="phead" style="padding-top:10px"><h1>Try searching</h1><p>Ask for what you feel like doing, not just event names.</p></div>
    <div class="suggest">
      ${SUGGESTIONS.map(s=>`<button class="sugg" onclick="state.q='${s.q}'; render()">
        <span class="sq">${s.e}</span>${s.q}<span class="sarr">${I.chev}</span></button>`).join("")}
    </div>`
  : res.length ? `
  ...
  : emptyBox("No matches","Try “this weekend”, “free”, “food” or a neighbourhood name.")}
  `;
}
...
function savedView(){
  ...
  return `
  <div class="phead" style="padding-top:22px"><h1>Saved</h1><p>Your shortlist. Decide later.</p></div>
...
function ticketsView(){
  ...
  ${state.orders.length ? state.orders.map(o=>{
    ...
    return `<button class="orderrow" style="width:calc(100% - 32px)" onclick="state.lastOrder=state.orders.find(x=>x.num==='${o.num}'); go('confirm')">
      <div class="oa" style="${art(e)}"></div>
      <div class="oi">
        <div class="when">${e.dateShort} · ${e.time}</div>
        <div class="onm">${esc(e.title)}</div>
        <div class="omt">${o.items.reduce((s,i)=>s+i.qty,0)} ticket(s) · ${esc(e.venue)}</div>
      </div>
      <span class="oq">${I.chev}</span>
    </button>`;
  }).join("")
  : emptyBox("No tickets yet","When you book something, your QR passes live here — offline too.","Find something to do","discover","🎟️")}`;
}
...
function profileView(){
  ...
  <div style="margin-top:18px; background:var(--surface); border-top:1px solid var(--line); border-bottom:1px solid var(--line)">
    ${[["📍","Location & radius","discover"],["🔔","Notifications",""],["💳","Payment methods",""],["❓","Help & contact",""]]
      .map(([q,l,v])=>`<button class="listrow" onclick="${v?`go('${v}')`:`toast('Not part of V1 — kept deliberately simple.')`}">
...
function emptyBox(h,p,cta,view,ico){
  return `<div class="empty">
    <div class="eico">${ico||"🔍"}</div><h3>${h}</h3><p>${p}</p>
    ${cta?`<button class="btn" onclick="go('${view}')">${cta}</button>`:""}
  </div>`;
}
function demoBar(){
  return `<div class="demobar">
    ...
    <div class="dls">
      ...
      <a class="demolink" href="organizer/">Organizer dashboard</a>
...
/* ---------- event detail ---------- */
function eventView(){
  const e = byId(state.eventId);
  if(!e) return discoverView();
  const sv = isSaved(e.id);
  const similar = EVENTS.filter(x=>x.id!==e.id && (x.cat===e.cat || x.tags.some(t=>e.tags.includes(t)))).slice(0,6);
  return `
  <div class="backbar">
  ...
  <div class="detailgrid">
    <div>
      ...
      <div class="factlist">
        ...
        <div class="fact"><span class="factico">📍</span><div>
          <div class="f1">${esc(e.venue)}</div><div class="f2">${esc(e.addr)} · <b>${e.km} km away</b></div></div></div>
        <div class="fact"><span class="factico">💰</span><div>
          <div class="f1">${e.from===0?"Free entry":"From "+$(e.from)}</div>
          <div class="f2">${esc(e.refund)}</div></div></div>
        <div class="fact"><span class="factico">🔥</span><div>
          <div class="f1">${kInterested(e.interested)} people interested</div><div class="f2">Based on saves and ticket activity</div></div></div>
      </div>
      ...
      <div class="dsec">
        ...
        <div class="expectgrid">
          ${e.expect.map(x=>`<div class="expect"><span class="ex">${x.e}</span>
            <div class="en">${esc(x.n)}</div><div class="ed">${esc(x.d)}</div></div>`).join("")}
        </div>
      </div>
      ...
      <div class="dsec">
        ...
        <p style="font-size:13px;color:var(--ink-3);margin-top:9px">${esc(e.venue)} · ${esc(e.area)} · ${e.km} km from you</p>
      </div>
      ...
      <div class="dsec">
        ...
        <div class="orgrow">
          ...
          <div style="min-width:0">
            <div class="on">${esc(e.org.name)}</div>
            <div class="om">${e.org.followers} followers · ${e.org.events} events · ${e.org.years} yrs</div>
          </div>
    ...
    <aside class="buybar">
      <div class="bp">
        <div class="bpl">${e.from===0?"Entry":"From"}</div>
        <div class="bpv">${e.from===0?"Free":$(e.from)}</div>
      </div>
  ...
  ${similar.length?`<section class="sec" style="margin-top:30px">
    ...
    <div class="rail">${similar.map(x=>card(x)).join("")}</div>
  </section>`:""}
...
function tabbar(){
  ...
  return tabs.map(([v,l,ico])=>{
    ...
    return `<button class="tabbtn${state.view===v?" on":""}" onclick="go('${v}')" aria-label="${l}">
...
function drawSheet(){
  ...
  let ov = el("sheet");
  if(!ov){
    ov = document.createElement("div"); ov.id="sheet"; ov.className="scrim-full";
    ...
    <div class="shead">
      ...
      <div class="sheadrow">
        <div style="flex:1;min-width:0">
          ...
          <div class="sd">${e.date} · ${e.time} · ${esc(e.venue)}</div>
        </div>
        <span class="hold">${I.lock} <span id="hold">${fmt(cart.secs)}</span></span>
        <button class="iconbtn" onclick="closeSheet()" aria-label="Close" style="width:32px;height:32px">✕</button>
      </div>
    </div>
    <div class="sbody">
      <div class="steps">
        <span class="sdot on"></span> Tickets
        <span class="sdot${cart.step===2?" on":""}" style="margin-left:6px"></span> Checkout
        <span class="sdot" style="margin-left:6px"></span> QR pass
      </div>
  ...
  ${e.tiers.map((tr,i)=>`
    <div class="tierbox${cart.qty[i]?" picked":""}${tr.gone?" gone":""}">
      <div class="ti">
        ...
        <div class="tp">${tr.price===0?"Free":$(tr.price)}${tr.price&&!tr.gone?` <span class="fee">+ ${$(fee(tr.price))} fees</span>`:""}</div>
      </div>
      ${tr.gone?`<span class="gonepill">Sold out</span>`:`
      <div class="step">
        <button onclick="bump(${i},-1)" ${cart.qty[i]?"":"disabled"} aria-label="Fewer">−</button>
        <span class="q">${cart.qty[i]}</span>
        <button onclick="bump(${i},1)" ${cart.qty[i]>=8?"disabled":""} aria-label="More">+</button>
      </div>`}
    </div>`).join("")}
  <div class="promorow">
    <input id="promo" placeholder="Promo code — try SAVE10" aria-label="Promo code"
...
function placeOrder(){
  ...
  setTimeout(()=>{
    ...
    const order = {num, eventId:cart.id, name, email, total:t.total, last4, brand,
  ...
  <div class="confwrap">
    ...
    ${o.items.map(i=>Array.from({length:i.qty},()=>{
      ...
      return `<div class="pass">
        <div class="passtop">
          <div class="qr"><canvas class="qrc" width="78" height="78" data-seed="${o.num}-${n}"></canvas></div>
          <div style="min-width:0">
        ...
        <div class="rip"></div>
        <div class="passbot"><span>${esc(o.name)}</span><span>${o.num}-${String(n).padStart(2,"0")}</span></div>
      </div>`;
    }).join("")).join("")}
    <div class="acctprompt">
      <span class="ai">🔐</span>
      <div style="flex:1">
        <h4>Keep this ticket safe</h4>
        <p>Create a free account so your pass survives a lost phone or cleared browser.</p>
      </div>
    ...
    <div class="confbtns">
      ...
      <button class="btn ghost" onclick="toast('Downloads an .ics calendar file in the real build')">Add to calendar</button>
...
addEventListener("scroll", ()=>{
  el("topbar")?.classList.toggle("stuck", scrollY > 6);
}, {passive:true});
if(location.hash){
  const id = location.hash.slice(1);
  if(byId(id)){ state.view="event"; state.eventId=id }
}
render();
